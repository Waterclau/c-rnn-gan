...
def generate_music(session, inputs, targets, initial_sequence, length, temperature):
    """ Generate a music sequence """
    sequence = initial_sequence
    # Initialize state
    state = session.run(initial_state, {initial_input: sequence[0:1]})
    # Generate sequence
    for i in range(length):
        # Compute probabilities for next note
        feed = {input_ph: sequence[-1:], initial_state: state, temperature_ph: temperature}
...
checkpoint_path = os.path.join(args.save_dir, 'model.ckpt-12.index')
